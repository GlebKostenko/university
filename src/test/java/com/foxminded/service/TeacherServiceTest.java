...
class TeacherServiceTest {
    ...
    void save_WhenAllIsRight_thenShouldBeNewRecord() {
        given(teacherRepository.save(new TeacherDTO("Lev","Landau")))
                .willReturn(new TeacherDTO(1L,"Lev","Landau"));
    ...
    void findById_WhenRecordExist_thenShouldFindThisRecord(){
        given(teacherRepository.findById(1L))
                .willReturn(Optional.of(new TeacherDTO(1L,"Lev","Landau")));
    ...
    void findAll_WhenRecordsExist_thenShouldBeNotEmptyResultList(){
        given(teacherRepository.findAll())
                .willReturn(Arrays.asList(new TeacherDTO(1L,"Lev","Landau")));
    ...
    void update_WhenRecordExist_thenRecordShouldBeUpdated() {
        ...
        verify(teacherRepository,times(1)).save(new TeacherDTO(1L,"Petr","Petrov"));
    }

    @Test
    void delete_WhenRecordExist_thenRecordShouldBeDeleted() {
        doNothing().when(teacherRepository).delete(new TeacherDTO(1L));
        teacherService.delete(new TeacherDTO(1L));
        verify(teacherRepository,times(1)).delete(new TeacherDTO(1L));
    }
}